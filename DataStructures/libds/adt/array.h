...
namespace ds::adt {
    ...
    template<typename T>
    Array<T>::Array(size_t size) : Array(Dimension(0, size)) {}
    ...
    bool Array<T>::equals(const ADT& other)
    {
        ...
        return otherArray != nullptr && base_ == otherArray->base_ && ADS<T>::equals(other);
    }
    ...
    template<typename T>
    CompactMatrix<T>::CompactMatrix(size_t size1, size_t size2) : CompactMatrix<T>(Dimension(0, size1), Dimension(0, size2)) {}
    ...
    bool CompactMatrix<T>::equals(const ADT& other)
    {
        ...
        return otherMatrix != nullptr && dimension1_ == otherMatrix->dimension1_ && dimension2_ == otherMatrix->dimension2_ && ADS<T>::equals(other);
    }
    ...
    bool CompactMatrix<T>::validateIndices(long long index1, long long index2) const
    {
        return index1 >= this->dimension1_.getBase() &&
            ...
            index2 < this->dimension2_.getBase() + this->dimension2_.getSize();
    }

    template<typename T>
    size_t CompactMatrix<T>::mapIndices(long long index1, long long index2) const
    {
        return static_cast<size_t>((index1 - this->dimension1_.getBase()) * this->dimension2_.getSize() + (index2 - this->dimension2_.getBase()));
    }
