...
namespace ds::adt {
    ...
    class GeneralList : virtual public List<T>, public ADS<T>
    {
    public:
    ...
    template<typename T, typename SequenceType>
    GeneralList<T, SequenceType>::GeneralList(const GeneralList& other) : ADS<T>(new SequenceType(), other) {}
    ...
    size_t GeneralList<T, SequenceType>::calculateIndex(T element)
    {
        ...
        typename SequenceType::BlockType* block = this->getSequence()->findBlockWithProperty([&](typename SequenceType::BlockType* b)
            {
                if (b->data_ == element)
                {
                    return true;
