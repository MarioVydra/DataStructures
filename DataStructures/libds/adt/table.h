...
namespace ds::adt {
    ...
    void UnsortedImplicitSequenceTable<K, T>::insert(K key, T data)
    {
        if (this->contains(key)) {
            this->error("Table already contains element asscociated with given key!");
        }
    ...
    T UnsortedImplicitSequenceTable<K, T>::remove(K key)
    {
        ...
        BlockType* blockWProperty = this->findBlockWithKey(key);
        if (blockWProperty == nullptr)
        {
            this->error("No such key");
        }
        T result = blockWProperty->data_.data_;
        BlockType* lastBlock = this->getSequence()->accessLast();
        if (blockWProperty != lastBlock)
        {
            std::swap(blockWProperty->data_, lastBlock->data_);
        }
    ...
    {
        BlockType* blockWKey = this->findBlockWithKey(key);
        if (blockWKey == nullptr)
        {
            this->error("No such key");
        }
        T result = blockWKey->data_.data_;
        BlockType* firstBlock = this->getSequence()->accessFirst();
        if (blockWKey != firstBlock)
        {
            std::swap(blockWKey->data_, firstBlock->data_);
        }
    ...
    {
        BlockType* blockWKey = nullptr;
        if (!this->tryFindBlockWithKey(key, 0, this->size(), blockWKey))
        {
            this->error("No such key!");
        }
        T result = blockWKey->data_.data_;
        if (this->getSequence()->accessFirst() == blockWKey)
        {
            this->getSequence()->removeFirst();
        }
        else {
            this->getSequence()->removeNext(*this->getSequence()->accessPrevious(*blockWKey));
        }
        return result;
    }
    ...
    auto HashTable<K, T>::HashTableIterator::operator++() -> HashTableIterator&
    {
        ...
        SynonymTableIterator oldIterator = *this->synonymIterator_;
        if (!(oldIterator != (**this->tablesCurrent_)->end()))
        {
            do {
                ++(*this->tablesCurrent_);
    ...
    void GeneralBinarySearchTree<K, T, BlockType>::insert(K key, T data)
    {
        ...
        {
            this->error("GeneralBinarySearchTree<K, T, BlockType>::insert: Key is present in table.");
        }
        else {
            BVSNodeType* newNode;
            if (node == nullptr) {
                ...
                newNode = &this->insertNode(key, node);
            }
    ...
    T GeneralBinarySearchTree<K, T, BlockType>::remove(K key)
    {
        ...
        if (this->tryFindNodeWithKey(key, node))
        {
            ...
            this->error("GeneralBinarySearchTree<K, T, BlockType>::remove:: Key is not present in table.");
        }
    }
