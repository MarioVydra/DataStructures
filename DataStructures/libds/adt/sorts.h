...
namespace ds::adt
{
    ...
    void QuickSort<T>::quick(amt::ImplicitSequence<T>& is, std::function<bool(const T&, const T&)> compare, size_t min, size_t max)
    {
        ...
        size_t rIndex = max;
        while (lIndex <= rIndex) {
            while (compare(is.access(lIndex)->data_, pivot)) {
                ++lIndex;
            }
            while (rIndex > 0 && compare(pivot, is.access(rIndex)->data_)) {
                --rIndex;
            }
            if (lIndex <= rIndex) {
                std::swap(is.access(lIndex)->data_, is.access(rIndex)->data_);
                ++lIndex;
                if (rIndex > 0) {
                    --rIndex;
                }
        ...
        if (min < rIndex)
        {
            this->quick(is, compare, min, rIndex);
        }
        if (lIndex < max)
        {
            this->quick(is, compare, lIndex, max);
        }
    }
