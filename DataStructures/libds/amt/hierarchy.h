...
namespace ds::amt {
	...
	typename Hierarchy<BlockType>::PreOrderHierarchyIterator& Hierarchy<BlockType>::PreOrderHierarchyIterator::operator++()
	{
		...
		{
			this->removePosition();
			if (this->currentPosition_ != nullptr) ++(*this);
		}
		return *this;
	}
	...
	typename BinaryHierarchy<BlockType>::InOrderHierarchyIterator& BinaryHierarchy<BlockType>::InOrderHierarchyIterator::operator++()
	{
		if (!this->currentPosition_->currentNodeProcessed_)
		{
			if (this->currentPosition_->currentSonOrder_ != BinaryHierarchy<BlockType>::LEFT_SON_INDEX && this->tryToGoToLeftSonInCurrentPosition())
			{
				this->savePosition(this->currentPosition_->currentSon_);
				++(*this);
			}
		...
		{
			if (this->currentPosition_->currentSonOrder_ != BinaryHierarchy<BlockType>::RIGHT_SON_INDEX && this->tryToGoToRightSonInCurrentPosition())
			{
				this->savePosition(this->currentPosition_->currentSon_);
				++(*this);
			}
